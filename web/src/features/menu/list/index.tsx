...
const useStyles = createStyles((theme, params: { position?: MenuPosition; itemCount: number; selected: number }) => ({
  ...
  container: {
    position: 'absolute',
    pointerEvents: 'none',
    marginTop: params.position === 'top-left' || params.position === 'top-right' ? 150 : 0,
    marginLeft: params.position === 'top-left' || params.position === 'bottom-left' ? 10 : 0,
    marginRight: params.position === 'top-right' || params.position === 'bottom-right' ? 10 : 0,
    marginBottom: params.position === 'bottom-left' || params.position === 'bottom-right' ? 10 : 0,
...
const ListMenu: React.FC = () => {
  ...
  return (
    <>
      ...
      <img className={classes.img3} src={bg2} alt=""/>
      </div>
      )}
      {visible && (
        <Tooltip
